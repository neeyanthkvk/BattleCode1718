               ...
               if(siteID[site.x][site.y]==-1)
                  if(viableSite[site.x][site.y])
                  {
                     for(int fact: factories)
                        if(gc.unit(fact).location().mapLocation().distanceSquaredTo(mMapLoc[site.x][site.y])<=2)
                           continue cycle;
                     if(gc.canSenseLocation(mMapLoc[site.x][site.y])&&gc.hasUnitAtLocation(mMapLoc[site.x][site.y]))
                     {
                        Unit toMove = gc.senseUnitAtLocation(mMapLoc[site.x][site.y]);
                        if(toMove!=null)
